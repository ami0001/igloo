...
<body>
 	<div id="container">
		...
		<div id="mainContent">
			...
			<tr>
			<td>1</td>
			<td><a href="recon/recon1.php">Spot the Blog</a></td>
			<td>Luffy</td>
			<td><?php require "config.php"; $query = mysqli_query($con,"SELECT * FROM answers") or die("Can not query DB."); while($row = mysqli_fetch_array($query)){echo $row[37];} ?></td>
			<td>100</td>
			<td><?php require "config.php";	$user = $_SESSION['username']; $query = mysqli_query($con,"SELECT * FROM members WHERE username = '$user'") or die("Can not query DB."); while($row = mysqli_fetch_array($query)){echo $row[21];} ?></td>
			</tr>
			</table>
